...
class DaE(torch.nn.Module):
    def __init__(self, vision, num_layers, num_nodes, num_semantics):
        ...
        cv = None
        if vision == 'mobilenet':
            cv = torchvision.models.mobilenet_v2(pretrained=False)
            cv.classifier[1] = torch.nn.Linear(cv.last_channel, num_nodes)
        elif vision == 'resnet':
            cv = torchvision.models.resnet152(pretrained=False)
            cv.fc = torch.nn.Linear(cv.fc.in_features, num_nodes)
        elif vision == 'vit':
            cv = timm.create_model('vit_base_patch16_224', pretrained=False, num_classes=num_nodes)
        self.vision = cv
